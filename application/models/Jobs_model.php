...
class Jobs_model extends CI_model {
    ...
    public function get_jobs_status($status, $date) //abslutely  should not have my dates hardcoded into the query but just for the sake of testing
    {
//        $query = $this->db->query("select * from jobs where status = '$status' and Date_Format(date_req, \"%m\")>=1 and Date_Format(date_req, \"%y\")>=14");
        $query = $this->db->query("select *, (select name from custies where jobs.cust_id = custies.cust_id) as name from jobs where status = '$status' and Date_Format(date_req, \"%m\")>=1 and Date_Format(date_req, \"%y\")>=14");
